...
{
    [Serializable]
    public class EntityDemoData : TargetableObjectData
    {
        [SerializeField]
        ...
        public EntityDemoData(int entityId, int typeId, CampType camp) : base(entityId, typeId, camp)
        {
            ...
            if (drAircraft == null)
            {
                Log.Error("加载游戏对象ID:[{0}]数据为空： " , entityId);
                return;
            }
