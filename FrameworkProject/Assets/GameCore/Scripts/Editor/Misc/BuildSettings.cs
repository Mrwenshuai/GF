...
namespace UnityGameFramework.Editor
{
    ...
    internal static class BuildSettings
    {
        ...
        {
            s_ConfigurationPath = Type.GetConfigurationPath<BuildSettingsConfigPathAttribute>() ?? Utility.Path.GetCombinePath(Application.dataPath, "GameMain/Configs/BuildSettings.xml");
            s_DefaultSceneNames.Clear();
            s_SearchScenePaths.Clear();

            if (!File.Exists(s_ConfigurationPath))
            {
                Debug.LogError("找不到配置路径： " + s_ConfigurationPath);
                return;
            }
