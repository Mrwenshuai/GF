...
namespace GameFramework
{
    ...
    internal sealed class TaskPool<T> where T : ITask
    {
        ...
        {
            foreach (var item in m_WaitingTasks)
            {
                if (item.SerialId == serialId)
                {
                    m_WaitingTasks.Remove(item);
                    return item;
                }
            }
            foreach (var item in m_WorkingAgents)
            {
                if (item.Task.SerialId == serialId)
                {
                    item.Reset();
                    m_FreeAgents.Push(item);
                    m_WorkingAgents.Remove(item);
                    return item.Task;
                }
            }
